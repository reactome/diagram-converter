...
/**
 * Note added by G.W. for the web-based curator tool conversion from gk_central. For this purpose, this application will and should run
 * once. It will generate diagram JSON for both PathwayDiagram and their represented Pathway instances. All files will be output into
 * the same folder. This folder should be moved to the server with the name as configured in curator-tool-ws. 
...
//        Integer version = ReactomeGraphCore.getService(GeneralService.class).getDBInfo().getVersion();
        // This number should be determined manually by checking either database (rel or graph).
        Integer version = 96; // Bypass the version fetch since we don't have for the gk_central converted database.
